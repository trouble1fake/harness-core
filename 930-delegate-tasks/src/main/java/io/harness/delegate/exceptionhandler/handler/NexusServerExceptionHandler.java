...
import io.harness.exception.runtime.InvalidNexusRegistryCredentialsRuntimeException;
import io.harness.exception.runtime.NexusRegistryInvalidImageRuntimeRuntimeException;
...
@Singleton
public class NexusServerExceptionHandler implements ExceptionHandler {
  public static Set<Class<? extends Exception>> exceptions() {
    return ImmutableSet.<Class<? extends Exception>>builder().add(NexusServerRuntimeException.class).build();
  }
  @Override
  public WingsException handleException(Exception exception) {
    NexusServerRuntimeException ex = (NexusServerRuntimeException) exception;
    if (ex instanceof InvalidNexusRegistryCredentialsRuntimeException) {
      ...
      return new HintException(
          HintException.HINT_NEXUS_ACCESS_DENIED, new InvalidCredentialsException(exception.getMessage(), USER));
    } else if (ex instanceof NexusRegistryInvalidImageRuntimeRuntimeException) {
      if (GlobalContextManager.get(MdcGlobalContextData.MDC_ID) != null) {
        Map<String, String> imageDetails =
            ((MdcGlobalContextData) GlobalContextManager.get(MdcGlobalContextData.MDC_ID)).getMap();
        return new ExplanationException(String.format(ExplanationException.COMMAND_TRIED_FOR_ARTIFACT,
            ...
            new ExplanationException(String.format(ExplanationException.IMAGE_METADATA_NOT_FOUND,
                                         ...
                                         imageDetails.get(ExceptionMetadataKeys.CONNECTOR.name())),
                new HintException(HintException.HINT_INVALID_IMAGE_REFER_LINK_NEXUS_REGISTRY,
                    new ImageNotFoundException(ex.getMessage(), USER))));
      }
      return new HintException(
          HintException.HINT_NEXUS_IMAGE_NAME, new ImageNotFoundException(exception.getMessage(), USER));
    } else if (ex instanceof NexusRegistryInvalidTagRuntimeRuntimeException) {
      if (GlobalContextManager.get(MdcGlobalContextData.MDC_ID) != null) {
        Map<String, String> imageDetails =
            ((MdcGlobalContextData) GlobalContextManager.get(MdcGlobalContextData.MDC_ID)).getMap();
        return new ExplanationException(String.format(ExplanationException.COMMAND_TRIED_FOR_ARTIFACT,
            ...
            new ExplanationException(String.format(ExplanationException.IMAGE_TAG_METADATA_NOT_FOUND,
                                         ...
                                         imageDetails.get(ExceptionMetadataKeys.CONNECTOR.name())),
                new HintException(HintException.HINT_INVALID_TAG_REFER_LINK_NEXUS_REGISTRY,
                    new io.harness.exception.InvalidTagException(ex.getMessage(), USER))));
      }
      return new HintException(HintException.HINT_INVALID_TAG_REFER_LINK_NEXUS_REGISTRY,
          new io.harness.exception.InvalidTagException(ex.getMessage(), USER));
    } else {

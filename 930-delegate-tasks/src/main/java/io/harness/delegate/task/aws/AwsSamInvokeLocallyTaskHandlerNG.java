...
@Slf4j
@OwnedBy(CDP)
public class AwsSamInvokeLocallyTaskHandlerNG extends AwsSamAbstractTaskHandler {
  ...
  public AwsSamTaskNGResponse executeTaskInternal(AwsSamTaskParameters taskParameters, String delegateId, String taskId,
    ...
    try (PlanJsonLogOutputStream planJsonLogOutputStream = new PlanJsonLogOutputStream()) {
      ...
      String samInvokeLocallyCommand = format("%s sam local invoke", awsSecretsExportCommand);
      CliResponse samInvokeLocallyCliResponse =
          awsSamClient.runCommand(samInvokeLocallyCommand, 120000l, envVariables, awsSamAppDirectory, logCallback);

      if (!UnitStatus.SUCCESS.equals(samInvokeLocallyCliResponse.getCommandExecutionStatus().getUnitStatus())) {
        return AwsSamTaskNGResponse.builder()
            .commandExecutionStatus(CommandExecutionStatus.FAILURE)
            .errorMessage(samInvokeLocallyCliResponse.getError())
            .customMessage("Sam build failed with output => " + samInvokeLocallyCliResponse.getOutput())
            .outputs(samInvokeLocallyCliResponse.getOutput())
            .build();
      }

...
public class ValidateSecretManagerConfigurationTask extends AbstractDelegateRunnableTask {
    ...
    public ValidateSecretManagerConfigurationTask(DelegateTaskPackage delegateTaskPackage, ILogStreamingTaskClient logStreamingTaskClient, Consumer<DelegateTaskResponse> consumer, BooleanSupplier preExecute) {
    ...
    private ValidateSecretManagerConfigurationTaskResponse run(ValidateSecretManagerConfigurationTaskParameters parameters) {
    ...
    private ValidateSecretManagerConfigurationTaskResponse runVaultTask(ValidateSecretManagerConfigurationTaskParameters parameters) {
        EncryptionConfig encryptionConfig = parameters.getEncryptionConfig();
        VaultEncryptor vaultEncryptor = vaultEncryptorsRegistry.getVaultEncryptor(encryptionConfig.getEncryptionType());
        boolean isConfigValid = vaultEncryptor.validateSecretManagerConfiguration(encryptionConfig.getAccountId(),encryptionConfig);
        return ValidateSecretManagerConfigurationTaskResponse.builder().isConfigurationValid(isConfigValid).build();
    }

    private ValidateSecretManagerConfigurationTaskResponse runKmsTask(ValidateSecretManagerConfigurationTaskParameters parameters) {
        EncryptionConfig encryptionConfig = parameters.getEncryptionConfig();

...
public class EncryptDecryptHelperImpl implements EncryptDecryptHelper {
  ...
  @Override
  public EncryptedRecord encryptContentOptimized(
      byte[] content, String name, EncryptionConfig config, DelegateFile delegateFile) throws IOException {
    EncryptedRecord encryptedRecord = encryptContent(content, name, config);
    if (KMS == config.getType()) {
      uploadEncryptedPlanToFileStorage((EncryptedRecordData) encryptedRecord, delegateFile);
    }
  ...
  @Override
  public byte[] getDecryptedContentOptimized(EncryptionConfig config, EncryptedRecord record, String accountId)
      throws IOException {
  ...
  private void uploadEncryptedPlanToFileStorage(EncryptedRecordData encryptedRecordData, DelegateFile delegateFile)
      throws IOException {

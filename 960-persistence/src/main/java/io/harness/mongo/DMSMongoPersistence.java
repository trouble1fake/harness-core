...
public class DMSMongoPersistence implements DMSPersistence {
    ...
    private Map<String, DMSMongoPersistence.Info> storeInfo = new HashMap<>();
    ...
    @Override
    public <T extends PersistentEntity> Query<T> createQuery(Class<T> cls, Set<HQuery.QueryChecks> queryChecks) {
    ...
    public <T extends PersistentEntity> Query<T> createQueryForCollection(
            String collectionName, Set<HQuery.QueryChecks> queryChecks) {
    ...
    public <T extends PersistentEntity> T upsert(
        ...
        if (CreatedByAware.class.isAssignableFrom(query.getEntityClass())) {
            setUnsetOnInsert(updateOperations, SampleEntity.SampleEntityKeys.createdBy, userProvider.activeUser());
        }

        if (CreatedAtAware.class.isAssignableFrom(query.getEntityClass())) {
            updateOperations.setOnInsert(SampleEntity.SampleEntityKeys.createdAt, currentTime);
        }
    ...
    private <T extends PersistentEntity> void onEntityUpdate(
        ...
        if (entity instanceof UpdatedByAware) {
            MongoUtils.setUnset(updateOperations, SampleEntity.SampleEntityKeys.lastUpdatedBy, userProvider.activeUser());
        }
        if (entity instanceof UpdatedAtAware) {
            updateOperations.set(SampleEntity.SampleEntityKeys.lastUpdatedAt, currentTime);
        }

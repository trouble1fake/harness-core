...
public class DMSMongoModule extends AbstractModule {
    private static volatile DMSMongoModule instance;

    static DMSMongoModule getInstance() {
        if (instance == null) {
            instance = new DMSMongoModule();
        }
        return instance;
    }
    ...
    @Singleton
    public static MongoClientOptions getDefaultMongoClientOptions(DMSMongoConfig mongoConfig) {
    ...
    public static AdvancedDatastore createDatastore(Morphia morphia, String uri) {
        DMSMongoConfig mongoConfig = DMSMongoConfig.builder().build();
    ...
    private static MongoClientOptions getMongoSslContextClientOptions(DMSMongoConfig mongoConfig) {
    ...
    private static void validateSSLMongoConfig(DMSMongoConfig mongoConfig) {
        MongoSSLConfig mongoSSLConfig = mongoConfig.getMongoSSLConfig();
    ...
    @Singleton
    public AdvancedDatastore primaryDatastore(DMSMongoConfig mongoConfig, @Named("morphiaClasses") Set<Class> classes,
    ...
    @Singleton
    public MongoClient getLocksMongoClient(DMSMongoConfig mongoConfig) {
        MongoClientURI uri;
    ...
    @Named("locksDatabase")
    @Singleton
    public String getLocksDatabase(DMSMongoConfig mongoConfig) {
        MongoClientURI uri;
        if (isNotEmpty(mongoConfig.getLocksUri())) {
            uri = new MongoClientURI(
                    mongoConfig.getLocksUri(), MongoClientOptions.builder(getDefaultMongoClientOptions(mongoConfig)));

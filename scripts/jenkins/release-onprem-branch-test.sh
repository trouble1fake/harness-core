...
git init
git clone https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/wings-software/portal.git
cd portal
git checkout $SAAS_BRANCH

onprem=`git branch -r | grep "release/on-prem" |grep ".xx$"| tail -5`

echo "onprem is : ${onprem}"

...
if [ "${RUN_BAZEL_TESTS}" == "true" ]; then
  ...
  bazel ${bazelrc} test --cache_test_results=${CACHE_TEST_RESULTS} --define=HARNESS_ARGS=${HARNESS_ARGS} --keep_going ${BAZEL_ARGUMENTS} -- //... -//product/... -//commons/... -//200-functional-test/... -//190-deployment-functional-tests/... -//360-cg-manager/... -//400-rest/...
  exit 0
fi

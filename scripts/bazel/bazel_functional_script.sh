...
if [ "${RUN_BAZEL_FUNCTIONAL_TESTS}" == "true" ]; then
  ...
  bazel run --spawn_strategy=standalone ${GCP} ${BAZEL_ARGUMENTS} 230-model-test:app &
  MANAGER_PID=$!
  ...
  bazel test --spawn_strategy=standalone --test_timeout=900 --keep_going ${GCP} ${BAZEL_ARGUMENTS} -- //200-functional-test:io.harness.functional.DummyFirstFunctionalTest || true

  #TODO: https://harness.atlassian.net/browse/BT-434
  bazel test --spawn_strategy=standalone --keep_going ${GCP} ${BAZEL_ARGUMENTS} --jobs=3 -- //200-functional-test/... \

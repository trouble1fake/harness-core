...
public class WebhookTriggerFilterUtils {
  ...
  public boolean checkIfCustomHeaderConditionsMatch(
    ...
    for (TriggerEventDataCondition webhookHeaderCondition : headerConditions) {
      ...
      headerConditionKey = convertToLowerCase(headerConditionKey);
  ...
  public String convertToLowerCase(String headerConditionKey) {
    try {
      Pattern p = Pattern.compile("<\\+trigger.header\\[(\\'|\")(.*?)(\\'|\")\\]>");
      Matcher m = p.matcher(headerConditionKey);

      if (m.find()) {
        String key = m.group(2);
        headerConditionKey = new StringBuilder("<+trigger.header['").append(key.toLowerCase()).append("']>").toString();
      }
    } catch (Exception e) {
      log.error("Failed while converting HeaderKey: " + headerConditionKey
              + " to lower case format. Continuing with key as is",
          e);
    }

    return headerConditionKey;
  }

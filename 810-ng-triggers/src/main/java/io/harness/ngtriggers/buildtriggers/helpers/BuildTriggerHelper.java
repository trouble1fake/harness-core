...
public class BuildTriggerHelper {
  ...
  public Optional<PipelineEntity> fetchPipelineForTrigger(NGTriggerEntity ngTriggerEntity) {
    return pmsPipelineService.get(ngTriggerEntity.getAccountId(), ngTriggerEntity.getOrgIdentifier(),
        ngTriggerEntity.getProjectIdentifier(), ngTriggerEntity.getIdentifier(), false);
  }
  ...
  public void validateBuildType(BuildTriggerOpsData buildTriggerOpsData) {
    ...
    buildTriggerOpsData.getTriggerSpecMap().entrySet().stream().forEach(
        entry -> exprMap.put(entry.getKey(), entry.getValue()));
  ...
  public Map<String, Object> convertMapForExprEvaluation(Map<String, JsonNode> triggerSpecMap) {
    ...
    triggerSpecMap.entrySet().forEach(entry -> map.put(entry.getKey(), entry.getValue()));
    return map;
  }
  ...
  public BuildTriggerOpsData generateBuildTriggerOpsDataForArtifact(TriggerDetails triggerDetails, String pipelineYml)
    ...
    String buildRef = triggerArtifactSpecMap.get("manifestRef").asText();
    List<String> keys = Arrays.asList(
  ...
  public void validatePollingItemForHelmChart(PollingItem pollingItem) {
    if (isBlank(pollingItem.getConnectorRef()) || "<+input>".equals(pollingItem.getConnectorRef())) {
      throw new InvalidRequestException("");
    }

    if (pollingItem.getPayloadType().hasHttpHelmPayload()) {
      // chartName should be there
      String chartName = pollingItem.getPayloadType().getHttpHelmPayload().getChartName();

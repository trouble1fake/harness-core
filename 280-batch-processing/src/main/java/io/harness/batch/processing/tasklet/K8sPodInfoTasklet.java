...
public class K8sPodInfoTasklet implements Tasklet {
  ...
  @Autowired private ClusterRecordService clusterRecordServiceCG;
  ...
  public Boolean isCurrentGenCluster(String clusterId) {
    io.harness.ccm.cluster.entities.ClusterRecord clusterRecordCG = cloudToHarnessMappingService.getClusterRecord(clusterId);
  ...
  public Boolean isNextGenCluster(String clusterId) {
    ClusterRecord clusterRecordNG = clusterRecordServiceNG.get(clusterId);
  ...
  public InstanceInfo process(PublishedMessage publishedMessage) {
    ...
    if (isCurrentGenCluster(clusterId)) {
      harnessServiceInfo = harnessServiceInfoFetcher
                               ...
                               .orElse(null);
    } else if (isNextGenCluster(clusterId)) {
      Optional<HarnessServiceInfoNG> harnessServiceInfoNG1;
      log.info("accountId: {}, podInfo.getPodName(): {}, podInfo.getNamespace(): {}", accountId, podInfo.getPodName(), podInfo.getNamespace());
      harnessServiceInfoNG1 = RestCallToNGManagerClientUtils.execute(
          instanceNGResourceClient.getInstanceNGData(accountId, podInfo.getPodName(), podInfo.getNamespace()));

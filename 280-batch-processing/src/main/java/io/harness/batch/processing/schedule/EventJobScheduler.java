  ...
  public void runCfSampleJob() {
    accountShardService.getCeEnabledAccounts().forEach(
            account -> {
              ...
              boolean result = cfClient.boolVariation("cf-sample-flag", target, false);
              log.info(format("The feature flag cf-sample-flag resolves to %s for account %s", result, target.getName()));
            });
  }

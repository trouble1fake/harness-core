package io.harness.batch.processing.tasklet.support;

import static io.harness.batch.processing.writer.constants.K8sCCMConstants.K8SV1_RELEASE_NAME;

import io.harness.batch.processing.tasklet.util.CacheUtils;
import io.harness.ccm.commons.beans.HarnessServiceInfo;

import software.wings.beans.infrastructure.instance.Instance;
import software.wings.service.intfc.instance.CloudToHarnessMappingService;

import com.github.benmanes.caffeine.cache.Caffeine;
import com.github.benmanes.caffeine.cache.LoadingCache;
import com.google.inject.Inject;
import java.util.Map;
import java.util.Optional;
import java.util.concurrent.TimeUnit;
import lombok.Value;
import lombok.extern.slf4j.Slf4j;
import org.mongodb.morphia.query.Sort;
import org.springframework.stereotype.Component;

@Slf4j
@Component
public class HarnessServiceInfoFetcherNG extends CacheUtils {
  private final K8sLabelServiceInfoFetcher k8sLabelServiceInfoFetcher;

  public final LoadingCache<CacheKey, Optional<HarnessServiceInfo>> getHarnessServiceInfoCache;

  @Value
  private static class CacheKey {
    private String accountId;
    private String namespace;
    private String podName;
  }

  @Inject
  public HarnessServiceInfoFetcherNG(K8sLabelServiceInfoFetcher k8sLabelServiceInfoFetcher,
      CloudToHarnessMappingService cloudToHarnessMappingService) {
    this.k8sLabelServiceInfoFetcher = k8sLabelServiceInfoFetcher;
    this.getHarnessServiceInfoCache =
        Caffeine.newBuilder()
            .recordStats()
            .expireAfterAccess(24, TimeUnit.HOURS)
            .maximumSize(1_000)
            .build(key -> this.getHarnessServiceInfoNG(key.accountId, key.namespace, key.podName));
  }

  public Optional<HarnessServiceInfo> fetchHarnessServiceInfoNG(
      String accountId, String namespace, String podName, Map<String, String> labelsMap) {
    try {
      Optional<HarnessServiceInfo> harnessServiceInfo = Optional.empty();
      if (labelsMap.containsKey(K8SV1_RELEASE_NAME)) {
        harnessServiceInfo = getHarnessServiceInfoCache.get(new CacheKey(accountId, namespace, podName));
      }
      // TODO: Fetch from alternate ways
      return harnessServiceInfo;
    } catch (Exception ex) {
      log.error("Error while fetching data {}", ex);
      return Optional.ofNullable(null);
    }
  }

  public Optional<HarnessServiceInfo> getHarnessServiceInfoNG(String accountId, String namespace, String podName) {
    // CALL API
    return Optional.ofNullable(null);
  }
}

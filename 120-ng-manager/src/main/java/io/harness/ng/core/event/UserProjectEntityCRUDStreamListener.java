...
public class UserProjectEntityCRUDStreamListener implements MessageListener {
    ...
    private boolean processOrganizationBasedUserDeleteEvent(OrganizationEntityChangeDTO organizationEntityChangeDTO) {
        ...
        userIds.forEach(userId ->{
            if (!ngUserService.removeUserFromScope(userId, scope, UserMembershipUpdateSource.SYSTEM)){
                log.error(String.format("Delete operation failed for users with accountIdentifier %s and identifier %s",
        ...
        if(success.get()){
            log.info(String.format(
                    "Successfully completed deletion for users having accountIdentifier %s and identifier %s",
                    accountIdentifier, id));
        }
        return success.get();
    }
    ...
    private boolean processProjectBasedUserDeleteEvent(ProjectEntityChangeDTO projectEntityChangeDTO) {
        ...
        userIds.forEach(userId ->{
            if (!ngUserService.removeUserFromScope(userId, scope, UserMembershipUpdateSource.SYSTEM)){
                log.error(String.format("Delete operation failed for users with accountIdentifier %s, orgIdentifier %s and identifier %s",

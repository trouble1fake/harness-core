...
@UtilityClass
@Slf4j
public class CENextGenManagerExecutor {
  ...
  public static void ensureServiceIsRunning(Class<?> clazz) throws IOException {
    if (!isHealthy()) {
      executeLocalManager(clazz);
    }
  }

  public static void executeLocalManager(Class<?> clazz) throws IOException {
    if (failedAlready) {
      return;
    }

    String directoryPath = Project.rootDirectory(clazz);
    final File lockfile = new File(directoryPath, "ce-nextgen");
    ...
    if (FileIo.acquireLock(lockfile, waiting)) {
      try {
        ...
        ProcessExecutor processExecutor = managerProcessExecutor(clazz);
        processExecutor.start();

        Poller.pollFor(waiting, ofSeconds(2), CENextGenManagerExecutor::isHealthy);
  ...
  public static ProcessExecutor managerProcessExecutor(Class<?> clazz) {
    String directoryPath = Project.rootDirectory(clazz);
    final File directory = new File(directoryPath);

    log.info("Execute the ce-nextgen manager from {}", directory);
  ...
  private static boolean isHealthy() {
    try {
      ...
      Setup.ceNextGenService().config(config).when().get("/health").then().statusCode(HttpStatus.SC_OK);

...
public class RancherTaskHelper {
    ...
    public RancherClusterDataResponse resolveRancherClusters(final RancherConfig rancherConfig,
    ...
    @NotNull
    private HttpInternalResponse makeRancherApi(final String httpMethod, final String url, RancherConfig rancherConfig) throws IOException {
        StringBuilder urlBuffer = new StringBuilder();
        urlBuffer.append(rancherConfig.getRancherUrl())
                .append(url);
        ...
        if (Objects.isNull(httpResponse) ||
                httpResponse.getHttpResponseCode() < 200 ||
    ...
    public KubernetesConfig createKubeconfig(final RancherConfig rancherConfig,
        ...
        RancherClusterDataResponse.ClusterData clusterData = rancherClusterData.getData().stream()
                .filter(cluster -> cluster.getName().equals(clusterName))
                .findFirst()
                .orElse(null);

        if (Objects.isNull(clusterData)) {
            throw new IllegalArgumentException("Unable to find cluster with display name: " + clusterName +
                    " in list of clusters fetched from Rancher");
        }
    ...
    private String generateKubeConfigFromRancher(RancherConfig rancherConfig,
        ...
        RancherGenerateKubeconfigResponse kubeconfigResponse = objectMapper.readValue(response.getHttpResponseBody(),
                RancherGenerateKubeconfigResponse.class);

...
public class ScmFetchFilesHelper {
  ...
  public GitFetchFilesResult fetchFilesFromRepoWithScm(
      GitFileConfig gitFileConfig, GitConfig gitConfig, List<String> filePathList, LogCallback executionLogCallback) {
  ...
  public void downloadFilesUsingScm(String manifestFilesDirectory, GitFileConfig gitFileConfig, GitConfig gitConfig,
    ...
    try {
      FileIo.createDirectoryIfDoesNotExist(manifestFilesDirectory);
      FileIo.waitForDirectoryToBeAccessibleOutOfProcess(manifestFilesDirectory, 10);
      for (FileContent fileContent : fileBatchContentResponse.getFileBatchContentResponse().getFileContentsList()) {
        writeFile(directoryPath, fileContent.getPath(), fileContent.getContent());
      }
    } catch (Exception ex) {
  ...
  private void writeFile(String directoryPath, String fileName, String fileContent) throws IOException {

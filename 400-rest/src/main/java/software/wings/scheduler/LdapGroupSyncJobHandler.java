...
public class LdapGroupSyncJobHandler implements MongoPersistenceIterator.Handler<SSOSettings> {
  ...
  public void registerIterators() {
    LdapSyncJobConfig ldapJobConfig = mainConfiguration.getLdapSyncJobConfig();
    persistenceIteratorFactory.createPumpIteratorWithDedicatedThreadPool(
        PersistenceIteratorFactory.PumpExecutorOptions.builder()
            .name("LdapGroupSyncTask")
            .poolSize(ldapJobConfig.getPoolSize())
            .interval(ofMinutes(ldapJobConfig.getSyncInterval()))
            .build(),
        SSOSettings.class,

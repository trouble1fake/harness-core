...
func (tdb *TimeScaleDb) Summary(ctx context.Context, accountId, orgId, projectId, pipelineId,
	...
	for rows.Next() {
		...
		if err != nil {
			...
			logs.Logger(ctx).Errorw("could not read summary response from DB", zap.Error(err))
			return types.SummaryResponse{}, err
		}
		total++
...
func (tdb *TimeScaleDb) GetTestCases(
	...
	if err != nil {
		logs.Logger(ctx).Errorw("could not query database for test cases", zap.Error(err))
	...
	for rows.Next() {
		...
		if err != nil {
			// Log error and return
			logs.Logger(ctx).Errorw("could not read test case response from DB", zap.Error(err))
			return types.TestCases{}, err
		}
...
func (tdb *TimeScaleDb) GetTestSuites(
	...
	if err != nil {
		logs.Logger(ctx).Errorw("could not query database for test suites", "error_msg", err)
	...
	for rows.Next() {
		...
		if err != nil {
			...
			logs.Logger(ctx).Errorw("could not read suite response from DB", zap.Error(err))
			return types.TestSuites{}, err
		}
		t.DurationMs = zdur.ValueOrZero()
	...
	if upsert == true {
		...
		*/
		...
		for rows.Next() {
			...
			err = rows.Scan(&zdur)
			if err != nil {
				logs.Logger(ctx).Errorw("could not get average test time", zap.Error(err))
				break
			}
...
func (tdb *TimeScaleDb) GetSelectionOverview(ctx context.Context, accountID, orgId, projectId, pipelineId,
	...
	defer rows.Close()
	if err != nil {
		logs.Logger(ctx).Errorw("could not query database for selection overview", zap.Error(err))
		return res, err
	}
	for rows.Next() {
		err = rows.Scan(&ztotal, &zsrc, &znew, &zupd, &ztt, &zts, &repo, &source, &target)
		if err != nil {
			logs.Logger(ctx).Errorw("could not read overview response from db", zap.Error(err))
			return res, err
		}
...
func (tdb *TimeScaleDb) WriteDiffFiles(ctx context.Context, accountID, orgId, projectId, pipelineId,
	...
	_, err := tdb.Conn.ExecContext(ctx, stmt, valueArgs...)
	if err != nil {
		logs.Logger(ctx).Errorw("could not write file information to database", zap.Error(err))
		return err
	}
	return nil
}
...
func (tdb *TimeScaleDb) GetDiffFiles(ctx context.Context, accountID, orgId, projectId, pipelineId,
	...
	if err != nil {
		logs.Logger(ctx).Errorw("could not query database for changed files", zap.Error(err))
		return res, err
	}
	for rows.Next() {

...
func (cgs *CgService) queryHelper(targetBranch, repo string, pkgs, classes []string) ([]types.RunnableTest, error) {
	...
	err := mgm.Coll(&db.Node{}).SimpleFind(&nodes, bson.M{"$or": allowedPairs})
	if err != nil {
		return nil, err
	}
	if len(nodes) == 0 {
	...
	err = mgm.Coll(&db.Relation{}).SimpleFind(&relations,
	...
	tnodes := []db.Node{}
	err = mgm.Coll(&db.Node{}).SimpleFind(&tnodes,
...
func (cgs *CgService) GetTestsToRun(ctx context.Context, req types.SelectTestsReq) (types.SelectTestsResp, error) {
	...
	all := []db.Node{}
	err = mgm.Coll(&db.Node{}).SimpleFind(&all, bson.M{"type": "test", "vcs_info.branch": req.TargetBranch, "vcs_info.repo": req.Repo})
	if err != nil {
		return res, err
	}
...
func (cgs *CgService) UploadPartialCg(ctx context.Context, cg *ti.Callgraph, info db.VCSInfo, acc, org, proj string) error {
	nodes := make([]db.Node, len(cg.Nodes))
	rels := make([]db.Relation, len(cg.Relations))
	for i, node := range cg.Nodes {
		nodes[i] = *db.NewNode(node.ID, node.Package, node.Method, node.Params, node.Class, node.Type, info, acc, org, proj)
	}
	for i, rel := range cg.Relations {
		rels[i] = *db.NewRelation(rel.Source, rel.Tests, info, acc, org, proj)
	}
...
func (cgs *CgService) MergePartialCg(ctx context.Context, req types.MergePartialCgRequest) error {
	commit := req.Diff.Sha
...
func (cgs *CgService) getNodeIds(ctx context.Context, commit, branch, repo string, f bson.M) ([]int, error) {
	var nodes []db.Node
	var nIds []int
...
func (cgs *CgService) getRelIds(ctx context.Context, commit, branch, repo string, f bson.M) ([]int, error) {
...
func (cgs *CgService) mergeNodes(ctx context.Context, commit, branch, repo string, nodesToMove []int) error {
...
func (cgs *CgService) mergeRelations(ctx context.Context, commit, branch, repo string, sIDs []int, dIDs []int) error {
	...
	var srcRelation, destRelation []db.Relation
...
func (cgs *CgService) upsertNodes(ctx context.Context, nodes []db.Node, info db.VCSInfo) error {
...
func (cgs *CgService) upsertRelations(ctx context.Context, relns []db.Relation, info db.VCSInfo) error {
	...
	relToUpdate := make([]db.Relation, 0)
...
func getRelMap(src []db.Relation, dest []db.Relation) map[int][]int {

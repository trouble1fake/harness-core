...
func TestSendStatusErr(t *testing.T) {
	...
	for _, tc := range tests {
		...
		got := SendStepStatus(ctx, stepID, "", accountID, callbackToken, taskID, numRetries, timeTaken, status, "", nil, nil, 0, 0, log.Sugar())
...
func TestSendStatusClientCreateErr(t *testing.T) {
	...
	err := SendStepStatus(ctx, stepID, "test", accountID, callbackToken, taskID, numRetries, timeTaken, status, "", nil, nil, -1, -1, log.Sugar())
	assert.NotNil(t, err)

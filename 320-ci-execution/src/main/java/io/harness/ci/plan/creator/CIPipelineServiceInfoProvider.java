...
public class CIPipelineServiceInfoProvider implements PipelineServiceInfoProvider {
  ...
  public List<StepInfo> getStepInfo() {
    StepInfo runStepInfo = StepInfo.newBuilder()
                               ...
                               .setStepMetaData(StepMetaData.newBuilder().setFolderPath("Build").build())
                               .build();
    ...
    StepInfo ecrPushBuilds = StepInfo.newBuilder()
    ...
    StepInfo dockerPushBuild = StepInfo.newBuilder()
                                   .setName("Build and Push an image to Docker Registry")
                                   .setType(StepSpecTypeConstants.BUILD_AND_PUSH_DOCKER_REGISTRY)
                                   .setStepMetaData(StepMetaData.newBuilder().setFolderPath("Build").build())
                                   .build();

    StepInfo gcrPushBuilds = StepInfo.newBuilder()
                                 .setName("Build and Push to GCR")
                                 .setType(StepSpecTypeConstants.BUILD_AND_PUSH_GCR)
                                 .setStepMetaData(StepMetaData.newBuilder().setFolderPath("Build").build())
                                 .build();

    StepInfo uploadArtifactsToJfrogArtifact =
        StepInfo.newBuilder()
            .setName("Upload Artifacts to JFrog Artifactory")
            .setType(StepSpecTypeConstants.ARTIFACTORY_UPLOAD)
            .setStepMetaData(StepMetaData.newBuilder().setFolderPath("Artifacts").build())
            .build();

    StepInfo ecrPushArtifacts = StepInfo.newBuilder()
                                    .setName("Build and Push to ECR")
                                    .setType(StepSpecTypeConstants.BUILD_AND_PUSH_ECR)
                                    .setStepMetaData(StepMetaData.newBuilder().setFolderPath("Artifacts").build())
                                    .build();

    StepInfo gcrPushArtifacts = StepInfo.newBuilder()
                                    .setName("Build and Push to GCR")
                                    .setType(StepSpecTypeConstants.BUILD_AND_PUSH_GCR)
                                    .setStepMetaData(StepMetaData.newBuilder().setFolderPath("Artifacts").build())
                                    .build();
    ...
    StepInfo dockerPushArtifacts = StepInfo.newBuilder()
                                       .setName("Build and Push an image to Docker Registry")
                                       .setType(StepSpecTypeConstants.BUILD_AND_PUSH_DOCKER_REGISTRY)
                                       .setStepMetaData(StepMetaData.newBuilder().setFolderPath("Artifacts").build())
                                       .build();
    ...
    stepInfos.add(gcrPushArtifacts);
    stepInfos.add(uploadToGCS);
    stepInfos.add(ecrPushBuilds);
    stepInfos.add(ecrPushArtifacts);
    ...
    stepInfos.add(dockerPushArtifacts);
    ...
    stepInfos.add(uploadArtifactsToJfrogArtifact);

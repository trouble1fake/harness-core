...
public class ArtifactoryRegistryServiceImpl implements ArtifactoryRegistryService {
  ...
  @Override
  public List<BuildDetailsInternal> getVersions(NexusRequest nexusConfig, String repoName) {
    String basicAuthHeader = Credentials.basic(nexusConfig.getUsername(), String.valueOf(nexusConfig.getPassword()));

    //        Map<String, String> response =  nexusClient.getRepositories(basicAuthHeader, repoName,null);
    Map<String, String> response = nexusClient.getRepositories(nexusConfig, "Docker");
    List<BuildDetailsInternal> list = new ArrayList<>();
    list.add(BuildDetailsInternal.builder().buildUrl("asd").number("1.2").uiDisplayName("1.2").build());
  ...
  //
  @Override
  public List<BuildDetailsInternal> getBuilds(NexusRequest nexusConfig, String imageName, int maxNumberOfBuilds) {
    return getVersions(nexusConfig, "todolist");
  ...
  //    return dockerRegistryUtils.getLabels(dockerConfig, registryRestClient, getToken, authHeader, imageName,
  //    buildNos);
  //  }
  //
  //  @Override

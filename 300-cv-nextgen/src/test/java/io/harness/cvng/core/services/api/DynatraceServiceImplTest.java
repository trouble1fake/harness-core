...
public class DynatraceServiceImplTest extends CategoryTest {
    ...
    @InjectMocks
    private DynatraceService classUnderTest = new DynatraceServiceImpl();
    ...
    private static final List<DynatraceServiceDTO> mockedServices =
            Arrays.asList(createDynatraceServiceResponse("Service 1", "service_1_entity_id", null),
    ...
    private static final List<DynatraceMetricDTO> mockedMetrics = Arrays.asList(
            createDynatraceMetricDTO("Response time", "builtin:service.response.time", "MicroSecond"),
    ...
    public void testGetAllDynatraceServices() {
        ...
        assertThat(dynatraceServices.size()).isEqualTo((int) mockedServices.stream().filter(service -> service.getServiceMethodIds() != null).count());
    }
    ...
    private static DynatraceMetricDTO createDynatraceMetricDTO(
            String displayName, String metricId, String unit) {
        return DynatraceMetricDTO.builder()
                .displayName(displayName)
                .metricId(metricId)
                .unit(unit)
                .build();

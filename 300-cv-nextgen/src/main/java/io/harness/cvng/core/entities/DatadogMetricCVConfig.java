...
public class DatadogMetricCVConfig extends MetricCVConfig {
  ...
  protected void validateParams() {
    checkNotNull(
        metricInfoList, generateErrorMessageFromParam(DatadogMetricCVConfig.DatadogCVConfigKeys.metricInfoList));
    metricInfoList.forEach(metricInfo -> {
      if (metricInfo.getServiceInstanceIdentifierTag() != null) {
        checkNotNull(
            metricInfo.getGroupingQuery(), generateErrorMessageFromParam(MetricInfo.MetricInfoKeys.groupingQuery));
      }
    });
  }

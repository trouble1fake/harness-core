...
public class PipelineServiceApplication extends Application<PipelineServiceConfiguration> {
  ...
  private static void registerObservers(Injector injector) {
    ...
    for (Class<? extends PipelineActionObserver> klass : PipelineActionObserverRegistry.getObservers()) {
      pmsPipelineService.getPipelineSubject().register(injector.getInstance(klass));
    }

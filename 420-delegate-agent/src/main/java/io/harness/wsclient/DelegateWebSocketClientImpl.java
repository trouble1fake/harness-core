...
public class DelegateWebSocketClientImpl implements DelegateWebSocketClient {
  ...
  private HttpClient configureHttpClient(String url)
    ...
    sslContextFactory.setEndpointIdentificationAlgorithm(HTTP);
    ...
    if (null != proxyHost && !isBlank(proxyHost.getHostName())) {
      sslContextFactory.setEndpointIdentificationAlgorithm(Http.getProxyScheme());
      httpClient = new HttpClient(sslContextFactory);
      Origin.Address address = new Origin.Address(proxyHost.getHostName(), proxyHost.getPort());
      HttpProxy proxy = new HttpProxy(address, sslContextFactory);
      httpClient.getProxyConfiguration().getProxies().add(proxy);

      if (!isBlank(Http.getProxyUserName())) {
        HttpAuthenticationStore store = new HttpAuthenticationStore();
        Authentication authentication =
            new BasicAuthentication(new URI(url), "<<ANY_REALM>>", Http.getProxyUserName(), Http.getProxyPassword());

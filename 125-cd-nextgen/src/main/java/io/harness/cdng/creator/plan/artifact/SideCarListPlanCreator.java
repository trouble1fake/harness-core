...
//  }
    ...
    public LinkedHashMap<String, PlanCreationResponse> createPlanForChildrenNodes(
        ...
        List<YamlNode> yamlNodes =
                Optional.of(sideCarsYamlField.getNode().asArray()).orElse(Collections.emptyList());
        Map<String,YamlNode> mapIdentifierWithYamlNode = yamlNodes.stream().collect(Collectors.toMap(e -> e.getField("sidecar").getNode().getIdentifier(), k -> k));

        for (Map.Entry<String, StepParameters> entry : sideCarsParametersMap.entrySet()) {
            addDependenciesForIndividualSideCar(sideCarsYamlField,entry.getKey(),entry.getValue(),mapIdentifierWithYamlNode,planCreationResponseMap);
        }

        return planCreationResponseMap;
    }

    private void addDependenciesForIndividualSideCar(YamlField sideCarsYamlField, String identifier, StepParameters stepParameters, Map<String, YamlNode> mapIdentifierWithYamlNode, LinkedHashMap<String, PlanCreationResponse> planCreationResponseMap) {
        YamlUpdates.Builder yamlUpdates = YamlUpdates.newBuilder();
        YamlField individualSideCar = SideCarsListArtifactsUtility.createIndividualSideCarsArtifactYamlFieldAndSetYamlUpdate(sideCarsYamlField,identifier,mapIdentifierWithYamlNode,yamlUpdates);
        ...
        PlanCreationResponse.PlanCreationResponseBuilder individualSideCarPlanResponse = PlanCreationResponse.builder().dependencies(
                DependenciesUtils.toDependenciesProto(dependenciesMap)
                        .toBuilder()
                        .putDependencyMetadata(individualSideCarPlanNodeId, Dependency.newBuilder().putAllMetadata(metadataDependency).build())
                        .build());
    ...
    private Map<String, ByteString> prepareMetadataForIndividualSideCarListArtifactPlanCreator(String individualSideCarPlanNodeId, StepParameters stepParameters) {
        Map<String, ByteString> metadataDependency = new HashMap<>();
        metadataDependency.put(YamlTypes.UUID, ByteString.copyFrom(kryoSerializer.asDeflatedBytes(individualSideCarPlanNodeId)));
        metadataDependency.put(
                PlanCreatorConstants.SIDECARS_PARAMETERS_MAP, ByteString.copyFrom(kryoSerializer.asDeflatedBytes(stepParameters)));
